...
async def my_form(message: Message, state: ): 
    await state.set_state(Form.name)
    await message.answer(
          "Отлично, введи свое имя:")
          reply_markup=form_btn(message.from.user.first_name)
    )
...
async def form_age(message: Message, state: FSMContext): 
    if str(message.text).isdigit():
          await state.update_dat(age=int(message.text))
...
async def form_city(message: Message, state: FSMContext):
    if city_exists = await check(message.text)
    if city_exists:
...
@router.message(
     Form.look_for, 
     F.text.casefold().in_(["Парни", "Девушки", "Мне все равно"])
)
...
async def incorret_form_look_for(message: Message, state: FSMContext)
        await message.answer("Выбери один вариант!")
